...
###   This script attempts to download the signature file SHA256SUMS.asc from teno.io
...
WORKINGDIR="/tmp/tenocoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://teno.io/bin/"
VERSIONPREFIX="tenocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tenocoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the TenoCoin Core binary release signing key installed?"
   fi
