...
The RPM spec file provided here is for TenoCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://teno.io/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[TenoCoin git repository](https://github.com/tenocoin/tenocoin) but are not part of
...
Currently only `Source10` is used, for the example `tenocoin.conf` file.
...
in the tenocoin git repository. At present only an SVG version of the TenoCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the tenocoin
...
needed when 0.12.1 is released, a proper fix is already in the TenoCoin git
master branch.
...
idea to build TenoCoin Core against a static version of that library compiled
...
The `tenocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `tenocoin-server` package. At this point, most RPM based
...
The `tenocoin-server` package should have SELinux support. How to properly do
...
TenoCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/tenocoin/). He has been
packaging TenoCoin for Fedora at least since 2012.
...
4. I renamed the `tenocoin` package that contains the Qt GUI to `tenocoin-core` as
that appears to be how the general population refers to it, in contrast to
`tenocoin-xt` or `tenocoin-classic`. I wanted to make sure the general population
