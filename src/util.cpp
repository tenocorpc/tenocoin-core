...
const char * const TENOCOIN_CONF_FILENAME = "tenocoin.conf";
const char * const TENOCOIN_PID_FILENAME = "tenocoin.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TenoCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TenoCoin
    // Mac: ~/Library/Application Support/TenoCoin
    // Unix: ~/.tenocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TenoCoin4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TenoCoin4";
#else
    // Unix
    return pathRet / ".tenocoin4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", TENOCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No tenocoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override tenocoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", TENOCOIN_PID_FILENAME));
