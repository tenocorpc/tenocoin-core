...
// Copyright (c) 2017 The TenoCoin developers
 ...
 * between tenocoind, and tenocoin-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), TENOCOIN_CONF_FILENAME));
    if (mode == HMM_TENOCOIND)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), TENOCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/tenocoindev/tenocoin-core>";
    const std::string URL_WEBSITE = "<https://teno.io>";
...
/** Sanity checks
 *  Ensure that TenoCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single TenoCoin process is using the data directory.
    ...
    if(GetBoolArg("-staking", true))
        threadGroup.create_thread(boost::bind(&TenoCoinStaker, boost::cref(chainparams)));
#endif
