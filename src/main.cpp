...
#if defined(NDEBUG)
# error "Tencoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Tencoin Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("tenocoin-scriptch");
...
//
// Commented - TenoCoin uses now version control
//
        ...
        for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
            ...
            if(block.vtx[0].vout[i].nValue > 0) {
                ...
                if(metadata[nPaymentRequestsCount].isStr()) {
                    ...
                    if(block.vtx[0].vout[i].nValue != prequest.nAmount || prequest.fState != CFund::ACCEPTED || parent.Address != CTenoCoinAddress(address).ToString())
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of TenoCoin, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of TenoCoin, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of TenoCoin, please update.";
            fObsolete = true;
        }
