        ...
        if(pblocktree->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CFund::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    if(CFund::FindProposal(prequest.proposalhash, parent)) {
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
                        CTenoCoinAddress addr(parent.Address);
                        if (!addr.IsValid())
                            continue;
...
void TenoCoinStaker(const CChainParams& chainparams)
{
    LogPrintf("TenoCoinStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tenocoin-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in TenoCoinStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running TenoCoinStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("TenoCoinStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("TenoCoinStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("TenoCoinStaker: ProcessNewBlock, block not accepted");
        }
    }
