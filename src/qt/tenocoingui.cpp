...
#include "main.h"
#include "tenocoingui.h"
#include "tenocoinunits.h"
...
const std::string TenoCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString TenoCoinGUI::DEFAULT_WALLET = "~Default";

TenoCoinGUI::TenoCoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void TenoCoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/tenocoin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a TenoCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tenocoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your TenoCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified TenoCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a tenocoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible TenoCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void TenoCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(TenoCoinUnits::Unit u, TenoCoinUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(TenoCoinUnits::name(u)), this);
...
void TenoCoinGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void TenoCoinGUI::createToolBars()
{
    if(walletFrame)
    {
...
void TenoCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool TenoCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void TenoCoinGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool TenoCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void TenoCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TenoCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void TenoCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void TenoCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void TenoCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void TenoCoinGUI::optionsClicked()
{
...
void TenoCoinGUI::showDebugWindow()
{
...
void TenoCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void TenoCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void TenoCoinGUI::openClicked()
{
...
void TenoCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void TenoCoinGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void TenoCoinGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void TenoCoinGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void TenoCoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void TenoCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TenoCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void TenoCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to TenoCoin network", "", count));
    if(walletFrame){
...
void TenoCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to TenoCoin network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to TenoCoin network..."));
...
void TenoCoinGUI::showVotingDialog()
{
...
void TenoCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("TenoCoin"); // default title
...
void TenoCoinGUI::changeEvent(QEvent *e)
{
...
void TenoCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TenoCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TenoCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(TenoCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void TenoCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TenoCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TenoCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TenoCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TenoCoinGUI::setEncryptionStatus(int status)
{
...
void TenoCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TenoCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TenoCoinGUI::detectShutdown()
{
...
void TenoCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void TenoCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(TenoCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void TenoCoinGUI::subscribeToCoreSignals()
{
...
void TenoCoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<TenoCoinUnits::Unit> units = TenoCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const TenoCoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(TenoCoinUnits::name(unit)));
    }
...
{
    setText(TenoCoinUnits::name(newUnits));
}
...
void TenoCoinGUI::toggleStaking()
{
...
void TenoCoinGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/teno-coin/?convert=EUR"));
...
void TenoCoinGUI::replyFinished(QNetworkReply *reply)
{
...
void TenoCoinGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CTenoCoinAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void TenoCoinGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.teno.io/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void TenoCoinGUI::updateStakingStatus()
{
    updateWeight();
